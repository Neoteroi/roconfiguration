...
class TestConfiguration:
    ...
    def test_dict_repr(self):
        config = Configuration(
            {"foo": True, "hello": 120, "child": {"abc": "Hello World"}}
        )
    ...
    def test_nested_overriding(self):
        config = Configuration({"a": {"b": 1, "c": 2, "d": {"e": 3, "f": 4}}})
    ...
    def test_reading_nested_list_values(self):
        config = Configuration(
            {"b2c": [{"tenant": "1"}, {"tenant": "2"}, {"tenant": "3"}]}
        )
    ...
    def test_add_environmental_variables(self):
        ...
        assert (
            config.hopefully_there_is_no_env_variable_with_such_name == "Hello, World"
        )
    ...
    @pytest.mark.parametrize("keyword", [("for",), ("del",)])
